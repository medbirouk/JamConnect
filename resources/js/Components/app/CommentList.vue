...
function deleteComment(comment) {
    ...
    axiosClient.delete(route('comment.delete', comment.id))
        .then(({ data }) => {
            const commentsArray = props.parentComment?.comments || props.data.comments;
            const index = commentsArray.findIndex(c => c.id === comment.id);
            if (index !== -1) {
    ...
    <div>
        <div v-for="comment of data.comments" :key="comment.id" class="mb-4">
            <div class="flex justify-between gap-2">
                ...
                <Menu v-if="authUser.id === comment.user.id" as="div" class="relative">
                    <MenuButton
                        class="w-6 h-6 rounded-full flex items-center justify-center hover:bg-black/5 transition">
                    ...
                    <transition enter-active-class="transition duration-100 ease-out"
                        ...
                        <MenuItems
                            class="absolute right-0 mt-2 w-32 origin-top-right rounded-md
                  bg-white dark:bg-slate-800 shadow-lg ring-1 ring-black/5 z-50 focus:outline-none text-gray-900 dark:text-white">

                            <div class="px-1 py-1 space-y-1">
                                <MenuItem v-slot="{ active }">
                                <button @click="startCommentEdit(comment)"
                                :class="[active ? 'bg-gray-100 dark:bg-slate-700' : '', 'flex items-center gap-2 px-2 py-1 text-sm w-full rounded']">

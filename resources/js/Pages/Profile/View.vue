...
import TabVideos from "@/Pages/Profile/TabVideos.vue";
import TabRoles from '@/Pages/Profile/TabRoles.vue';
...
const filteredFollowers = computed(() => {
    return props.followers.filter(user =>
        user.name?.toLowerCase().includes(searchFollowersKeyword.value.toLowerCase())
    )
})
const filteredFollowings = computed(() => {
    return props.followings.filter(user =>
        user.name?.toLowerCase().includes(searchFollowingsKeyword.value.toLowerCase())
    )
})
</script>
<template>
            ...
            <div class="border-t m-4 mt-0">
                <TabGroup>
                    <TabList class="flex bg-white dark:bg-slate-950 dark:text-white">
                        ...
                        <Tab v-slot="{ selected }" as="template">
                            <TabItem text="Videos" :selected="selected" />
                        </Tab>
                        <Tab v-slot="{ selected }" as="template">
                            <TabItem text="Roles" :selected="selected" />
                        </Tab>
                    ...
                    <TabPanels class="mt-2">
                        <TabPanel>
                            ...
                            <PostList v-if="posts && posts.data.length" :posts="posts" class="flex-1" />
                            <div v-else class="py-8 text-center dark:text-gray-100">
                                {{ isMyProfile
                                    ? 'You haven’t posted anything yet. Use the box above to create your first event!'
                        ...
                        <TabPanel>
                            ...
                            <div v-if="filteredFollowers.length" class="grid grid-cols-2 gap-3">
                                <UserListItem v-for="user of filteredFollowers" :user="user" :key="user.id"
                        ...
                        <TabPanel>
                            ...
                            <div v-if="filteredFollowings.length" class="grid grid-cols-2 gap-3">
                                <UserListItem v-for="user of filteredFollowings" :user="user" :key="user.id"
                                    class="shadow rounded-lg" />
                            </div>
                            <div v-else class="text-center py-8 dark:text-gray-100">
                        ...
                        <TabPanel>
                            <TabPhotos :photos="photos" :is-my-profile="isMyProfile" />
                        </TabPanel>
                        <TabPanel>
                            <TabVideos :videos="videos" :is-my-profile="isMyProfile" />
                        </TabPanel>
                        <TabPanel>
                            <TabRoles :roles="user.roles" :is-my-profile="isMyProfile" />
                        </TabPanel>
